...
ggplot(df.violins, aes(x = employment_status, y = PHQ4_sum, fill = employment_status)) +
  ...
  ylab("Self-reported stress and anxiety\n(PHQ4 Sum Score out of 16)") +
  theme(legend.position = "none",
        axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)),
        axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)))
  

mod_children <- lm(PHQ4_sum ~ household_children, data = df.violins)
summary(mod_children)

mod_emplo <- lm(PHQ4_sum ~ employment_status, data = df.violins)
summary(mod_emplo)

mod_emplo.gender <- lm(PHQ4_sum ~ employment_status + gender, data = df.violins)
summary(mod_emplo.gender)
mod_emplo.gender_int <- lm(PHQ4_sum ~ employment_status * gender, data = df.violins)
summary(mod_emplo.gender_int)

anova(mod_emplo, mod_emplo.gender, mod_emplo.gender_int)
...
mean_group <- df.violins %>% 
  group_by(gender, employment_status) %>% 
  summarize(group_mean = mean(PHQ4_sum, na.rm = TRUE))
...
df.forAnalysis <- covid_recode %>% 
  ...
  mutate(employment_status = factor(employment_status, levels = 
                                      c("Full time employment", "Part time employment",
                                  "Unemployed", "Not working"))) %>% 
  separate(endtime, into = c("date", "time"), sep = " ") %>% 
  separate(date, into = c("day", "month", "year"), sep = "/") %>% 
  mutate(year_month = factor(paste(year, month, sep = "_")))
...
ggplot(df.forAnalysis, aes(x = year_month, y = PHQ4_sum)) +
  stat_summary(fun = mean, geom = "point") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  stat_summary(fun = mean, geom = "line", aes(group = 1)) +
  theme_bw()

ggplot(df.forAnalysis, aes(x = year_month, y = PHQ4_sum, color = gender)) +
  stat_summary(fun = mean, geom = "point") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
