...
library(ggbeeswarm)
# library(tidylog)
setwd("/Users/alda/Documents/DS4A")
...
# Here we need to make week as categorical/factor before throwing it into the regression since it doesn't make sense to make them as numeric, cts variable 
df.forAnalysis <- df.forAnalysis %>% 
...
```{r}
df.forAnalysis_4mo <- df.forAnalysis %>% 
  filter( as.integer(week) < 16)

mod_4mo <- lm(PHQ4_sum ~ household_children*gender*employment_status + age + week, data = df.forAnalysis_4mo)

paste("Our sample size for this analysis:", length(mod$fitted.values), "participants")

export_summs(mod_4mo, error_format = "[{conf.low}, {conf.high}]", error_pos = "same", results = "asis")
```


```{r}
mod_women_4mo <- lm(PHQ4_sum ~ household_children*employment_status + age + week, data = filter(df.forAnalysis_4mo, gender == "Female"))
mod_men_4mo <- lm(PHQ4_sum ~ household_children*employment_status + age + week, data = filter(df.forAnalysis_4mo, gender == "Male"))
paste("Number of women:", length(mod_women_4mo$fitted.values), "and number of men:", length(mod_men_4mo$fitted.values))
export_summs(mod_women_4mo, mod_men_4mo, error_format = "[{conf.low}, {conf.high}]", error_pos = "same", results = "asis")

```

## Time Series 

I think it only makes sense to define a time series with one data point per time period. Since we have multiple observations per time period (multiple survey respondent for a given week) we can try to come up with an aggregate metric, e.g. PHQ4_sum added across all survey respondents, divided by the number of survey respondents (i.e. average PHQ4_sum for a given week across all respondents). 

```{r}
ts_df <- df.forAnalysis %>% 
  group_by(week) %>%
  summarise( avg_PHQ4_sum = mean(PHQ4_sum, na.rm=T)) %>% 
  ungroup

check_week <- df.forAnalysis %>% 
  mutate(date = as.Date(endtime, format="%m/%d/%Y")) %>% 
  distinct(date, week)

#create time series
phq4_ts <- ts(ts_df$avg_PHQ4_sum, start=c(2020,2,4), end=c(2021,7,9), frequency=52)
plot(phq4_ts)

# Seasonal decomposition
stl_fit <- stl(phq4_ts, s.window="period")
plot(stl_fit)
...
# Next step would be to try to fit TS models (ARIMA/Holts-Winter models)
```
